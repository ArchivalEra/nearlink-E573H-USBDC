...
### 📚 情报库（49 份研究文档）
星闪协议全套深挖：SSAP 方言对照（**OHOS 与设备固件同协议，移植路线实证**）、连接管理、数据面（DTAP/SDR）、安全配对、测距、标准参数（SLE 12Mbps/250µs/256 用户），外加 6 份 OpenSparklink 契约、8 份 WS63/HHD-01 批次与 17 份 OHOS 生态（AT 框架/SLE mesh/framework/HDI/SSAP 权威引擎/客户端/HID 遥控/SA 服务/测距全链路/雷达/DLI 传输/DTAP 数据面/NAI 管理/设备管理/GLE/BGTP）。
...
└── .scratch/            # wayfinder 决策图 + 49 份研究报告 + 师傅清单
```
...
| `scripts/` | 测试/验证/检查脚本 | — |
| `.scratch/nearlink-driver/lab-notes/` | **49 份研究报告**（SSAP 方言/CM/DTAP/SDR/SM/HADM/标准/OSPL/WS63/OHOS…） | 中英混合 |
