...
### 📚 情报库（32 份研究文档）
星闪协议全套深挖：SSAP 方言对照（**OHOS 与设备固件同协议，移植路线实证**）、连接管理、数据面（DTAP/SDR）、安全配对、测距、标准参数（SLE 12Mbps/250µs/256 用户），外加 6 份 OpenSparklink 契约报告（DLI/UAPI/CONN-FSM/SSAP/PHY/USB 传输）与 8 份 WS63/HHD-01 批次（官方 SSAP/连接/发现/HADM API、SLE 示例、构建烧录、芯片家族差异、板卡上手、社区项目）。
...
└── .scratch/            # wayfinder 决策图 + 32 份研究报告 + 师傅清单
```
...
| `scripts/` | 测试/验证/检查脚本 | — |
| `.scratch/nearlink-driver/lab-notes/` | **32 份研究报告**（SSAP 方言/CM/DTAP/SDR/SM/HADM/标准/OSPL/WS63…） | 中英混合 |
