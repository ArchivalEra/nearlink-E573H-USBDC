...
### 📚 情报库（25 份研究文档）
星闪协议全套深挖：SSAP 方言对照（**OHOS 与设备固件同协议，移植路线实证**）、连接管理、数据面（DTAP/SDR）、安全配对、测距、标准参数（SLE 12Mbps/250µs/256 用户），外加 6 份 OpenSparklink 契约报告（DLI/UAPI/CONN-FSM/SSAP/PHY/USB 传输）。
...
└── .scratch/            # wayfinder 决策图 + 25 份研究报告 + 师傅清单
```
...
| `scripts/` | 测试/验证/检查脚本 | — |
| `.scratch/nearlink-driver/lab-notes/` | **25 份研究报告**（SSAP 方言/CM/DTAP/SDR/SM/HADM/标准/OSPL…） | 中英混合 |
