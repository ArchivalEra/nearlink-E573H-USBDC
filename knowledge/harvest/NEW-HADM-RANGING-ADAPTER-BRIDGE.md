...
- **Per-call heap lifecycle with an asymmetric failure leak.** `iqDut`/`iqRtd` are `new[]`-allocated each call and deleted after the call (`:99-102,144-146`), but the early-return guard on allocation failure (`NL_CHECK_RETURN` at `:101-102`) leaves the first array unfreed if the second allocation fails. A single sounder calling this per frame leaks one array per failure. Copy-worthy pattern with that defect fixed.
